...
from src.rag.utils import singleton, get_float
from src.utils import get_project_base_directory
from src.utils import convert_bytes
from src.rag.utils.doc_store_conn import DocStoreConnection, MatchExpr, OrderByExpr, MatchTextExpr, MatchDenseExpr, \
    FusionExpr
from src.rag.nlp import is_english, rag_tokenizer

ATTEMPT_TIME = 2

logger = logging.getLogger('aioters_rag.es_conn')
...
class ESConnection(DocStoreConnection):
    def __init__(self):
        ...
        if int(v) < 8:
            ...
            logger.error(msg)
            raise Exception(msg)
        # 從 src/configs.py 獲取 ES mapping 配置
        from src.configs import get_settings
        app_settings = get_settings()
        self.mapping = app_settings.es_mapping
