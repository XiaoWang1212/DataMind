...
from src.rag.nlp import bullets_category, is_english,remove_contents_table, \
    hierarchical_merge, make_colon_as_title, naive_merge, random_choices, tokenize_table, \
    tokenize_chunks
from src.rag.nlp import rag_tokenizer
from src.rag.deepdoc.parser import PdfParser, DocxParser, PlainParser, HtmlParser

# Import tika parser for .doc files
try:
    ...
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        if parser is None:
            raise NotImplementedError(
                "Parsing .doc files requires tika-python. Install with: pip install tika")
        callback(0.1, "Start to parse.")
