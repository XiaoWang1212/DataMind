...
from src.rag.deepdoc.parser import DocxParser, ExcelParser, HtmlParser, JsonParser, MarkdownElementExtractor, MarkdownParser, PdfParser, TxtParser
from src.rag.deepdoc.parser.ppt_parser import AIoTersRAGPptParser
from src.rag.deepdoc.parser.figure_parser import VisionFigureParser, vision_figure_parser_figure_data_wrapper
from src.rag.deepdoc.parser.pdf_parser import PlainParser, VisionParser
from src.rag.nlp import concat_img, find_codec, naive_merge, naive_merge_with_images, naive_merge_docx, rag_tokenizer, tokenize_chunks, tokenize_chunks_with_images, tokenize_table
...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    elif re.search(r"\.(ppt|pptx)$", filename, re.IGNORECASE):
        callback(0.1, "Start to parse.")
        ppt_parser = AIoTersRAGPptParser()
        sections = ppt_parser(binary, from_page=from_page, to_page=to_page, callback=callback)
        sections = [(_, "") for _ in sections if _]
        callback(0.8, "Finish parsing.")

    else:
        raise NotImplementedError(
            "file type not supported yet(pdf, xlsx, doc, docx, txt, ppt, pptx supported)")
