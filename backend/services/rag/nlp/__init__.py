...
def naive_merge(sections: str | list, chunk_token_num=1024, delimiter="\n。；！？", overlapped_percent=15):
    from src.rag.deepdoc.parser.pdf_parser import AIoTersRAGPdfParser
    if not sections:
        return []
    if isinstance(sections[0], type("")):
        sections = [(s, "") for s in sections]
    ...
    def add_chunk(t, pos):
        ...
        if cks[-1] == "" or tk_nums[-1] > chunk_token_num * (100 - overlapped_percent)/100.:
            if cks:
                overlapped = AIoTersRAGPdfParser.remove_tag(cks[-1])
...
def naive_merge_with_images(texts, images, chunk_token_num=1024, delimiter="\n。；！？", overlapped_percent=15):
    from src.rag.deepdoc.parser.pdf_parser import AIoTersRAGPdfParser
...
def naive_merge_docx(sections, chunk_token_num=1024, delimiter="\n。；！？"):
    if not sections:
        return [], []
