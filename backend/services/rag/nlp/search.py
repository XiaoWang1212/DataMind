...
from src.rag.settings import TAG_FLD, PAGERANK_FLD
from src.rag.utils import rmSpace, get_float
from src.rag.nlp import rag_tokenizer, query
import numpy as np
from src.rag.utils.doc_store_conn import DocStoreConnection, MatchDenseExpr, FusionExpr, OrderByExpr


def index_name(uid): return f"aioters_rag_{uid}"
...
class Dealer:
    ...
    def search(self, req, idx_names: str | list[str],
        ...
        if not qst:
            ...
            print(f"\n=== QUESTION PROCESSING DEBUG ===")
            print(f"Original question: {qst}")
            print(f"Extracted keywords: {keywords}")
            print(f"matchText: {matchText}")
            if emb_mdl is None:
                ...
                # 計算權重：term_weight, vector_weight
                term_weight = 1.0 - vector_similarity_weight
                weights = f"{term_weight},{vector_similarity_weight}"
                print(f"\n=== SEARCH.PY DEBUG ===")
                print(f"vector_similarity_weight (input): {vector_similarity_weight}")
                print(f"term_weight: {term_weight}")
                print(f"weights string: {weights}")
                print(f"query vector dimension: {len(q_vec)}")
                fusionExpr = FusionExpr("weighted_sum", topk, {"weights": weights})
