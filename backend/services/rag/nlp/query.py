...
from src.rag.utils.doc_store_conn import MatchTextExpr
from src.rag.nlp import rag_tokenizer, term_weight, synonym
...
class FulltextQueryer:
    def __init__(self):
        ...
        # 傳遞 Redis 連接以啟用實時同義詞
        redis_client = REDIS_CONN.REDIS if REDIS_CONN.is_alive() else None
        self.syn = synonym.Dealer(redis=redis_client)
        self.query_fields = [
            "title_tks^10",
            "title_sm_tks^5",
            "important_kwd^30",
    ...
    def rmWWW(txt):
        patts = [
            (
                r"是*(怎麼辦|什麼樣的|哪家|一下|那家|請問|啥樣|咋樣了|什麼時候|何時|何地|何人|是否|是不是|多少|哪裡|怎麼|哪兒|怎麼樣|如何|哪些|是啥|啥是|啊|嗎|呢|吧|咋|什麼|有沒有|呀|誰|哪位|哪個)是*",
                "",
            ),
