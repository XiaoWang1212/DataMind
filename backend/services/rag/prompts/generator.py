...
import jinja2
import json_repair
from src.utils import hash_str2int
from src.rag.prompts.template import load_prompt
from src.rag.settings import TAG_FLD
from src.rag.utils import encoder, num_tokens_from_string
...
def _kb_prompt_sync(kbinfos, max_tokens, hash_id=False):
    """
    構建知識庫檢索結果的 prompt (同步版本，不包含文檔元數據)

    此函數用於向後兼容原專案的同步 ask() 函數。
    不包含文檔元數據功能，因為需要數據庫異步查詢。

    Args:
        kbinfos: 包含 chunks 的檢索結果
        max_tokens: 最大 token 數限制
        hash_id: 是否對 ID 進行哈希處理

    Returns:
        格式化的知識列表
    """
    # 第一步：計算 token 並確定可用的 chunks 數量
    contents = [get_value(ck, "content", "content_with_weight") for ck in kbinfos["chunks"]]
    kwlg_len = len(contents)
    used_token_count = 0
    chunks_num = 0
    ...
    for i, c in enumerate(contents):
        ...
        if chunk_tokens == 0:
            # Fallback: estimate tokens for Chinese text
            chunk_tokens = int(len(c) * 1.5)
        logging.debug(f"Chunk {i}: {len(c)} chars, {chunk_tokens} tokens, cumulative: {used_token_count + chunk_tokens}/{max_tokens}")
        used_token_count += chunk_tokens
        chunks_num += 1
        if max_tokens * 0.97 < used_token_count:
            chunks_num = i
            logging.warning(f"Not all the retrieval into prompt: {chunks_num}/{kwlg_len} (used {used_token_count} tokens)")
            break

    # 第二步：構建格式化的知識列表（不包含文檔元數據）
...
async def kb_prompt(kbinfos, max_tokens, hash_id=False, db=None):
    """
    構建知識庫檢索結果的 prompt
    ...
    # 第二步：獲取文檔元數據（如果提供了 db）
    docs = {}
    if db:
        try:
            doc_service = DocumentService()
            doc_ids = [get_value(ck, "doc_id", "document_id") for ck in kbinfos["chunks"][:chunks_num]]
            doc_list = await doc_service.get_by_ids(db=db, doc_ids=doc_ids)
            docs = {d.id: d.meta_fields or {} for d in doc_list}
            logging.debug(f"Loaded metadata for {len(docs)} documents")
...
def full_question(tenant_id=None, llm_id=None, messages=[], language=None, chat_mdl=None):
    ...
    from src.services.llm_service import TenantLLMService
