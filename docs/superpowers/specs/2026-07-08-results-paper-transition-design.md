...
- **導航架構:Tab + 巢狀路由。** Tab 外觀保留,點擊實際做 `router.push`,active 狀態由目前路由推導。
...
/results/report   → PaperView(論文 + 引用側欄)
/results/code     → CodeView(占位,之後再做)
```

外殼元件持有 Sidebar、返回鍵與 toolbar tabs,中間為 `<router-view>`。Toolbar 為三個 tab:總覽 / 報告 / 程式碼。

## 2. 元件拆分

```
frontend/src/views/results/
  ResultsLayout.vue      外殼:Sidebar + toolbar + router-view
  ResultsOverview.vue    metric cards + AI 洞察 + 模型比較表(自現有 ResultsPage 搬移)
  PaperView.vue          論文主體 + 引用側欄的容器
frontend/src/components/results/
  PaperSection.vue       單一章節渲染,含引用 highlight
  CitationPanel.vue      右側「來源文獻 + 檢索片段」卡片列表
```

現有 `views/ResultsPage.vue` 由 `ResultsLayout.vue` 取代;路由 `/results` 改為巢狀定義。
