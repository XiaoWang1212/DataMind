...
### 2. 論文編輯：無修改即可直接退出（已撤銷，見下方說明）

> **撤銷紀錄**：這個設計實作後在 `docs/superpowers/plans/2026-07-30-frontend-ux-fixes.md` 的 Task 2→3→4 中經歷兩次修法都無法可靠運作——根因是 Tiptap 編輯器會在掛載時、以及 `editable` 屬性第一次從 `false` 變 `true` 時各自正規化一次文件內容，這個正規化動作跟真正的使用者修改難以區分，導致「是否有修改」的判斷持續誤判。因兩次修法都失敗，最終決定撤銷這個功能（commit `631dc50`），`ModeSwitch` 恢復成進入編輯模式後無條件鎖定的原始行為。下方內容保留作為設計嘗試的歷史紀錄，**不代表目前的實際行為**。詳細過程見上述 plan 文件的 Task 3、Task 4。
