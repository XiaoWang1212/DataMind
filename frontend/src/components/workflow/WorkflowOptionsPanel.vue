<template>
  ...
  <section class="setting-area">
    ...
    <div v-if="selectedNode" class="panel">
      ...
      <div class="panel-body">
        ...
        <template v-if="selectedNode.id === 'dataTable'">
          <div class="form-row">
            ...
            <input id="preview-rows" v-model.number="localConfig.previewRows" type="number" min="1" />
          </div>
          ...
          <div class="preview-box" :style="previewBoxStyle">
            <table>
              <thead>
                <tr>
                  <th v-for="header in PREVIEW_HEADERS" :key="header">{{ header }}</th>
                </tr>
              </thead>
              <tbody>
        ...
        <template v-else>
          <div v-for="field in selectedNode.data.fields" :key="field.key" class="form-row">
            <label :for="field.key">{{ field.label }}</label>

            <input v-if="field.type === 'text'" :id="field.key" v-model="localConfig[field.key]" type="text" />
        ...
        <details v-if="selectedNode.id.startsWith('model')" class="details" open>
          <summary class="details__summary">模型參數</summary>
          <div class="details__content">
            <!-- modelMore 節點：列出已收合的模型 -->
            <template v-if="selectedNode.id === 'modelMore'">
              <div class="hint">
                已收合模型：Support Vector Machine、Naive Bayes、K-Nearest Neighbors ...
              </div>
            </template>
            <!-- 其餘模型節點：沒有額外參數時顯示提示 -->
      ...
      <!-- 操作按鈕：固定在面板底部 -->
      <div class="actions">
        <button class="btn btn-primary" type="button" @click="save">儲存設定</button>
      </div>
    </div>
  </section>
</template>
...
import { PREVIEW_HEADERS, PREVIEW_SOURCE_ROWS } from '@/constants/workflowData'
...
const emit = defineEmits<{
  (e: 'update-config', payload: { nodeId: string; config: Record<string, ConfigValue> }): void
}>()
